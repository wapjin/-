...
if __name__ == '__main__':
    ...
    while True:
        ...
        time.sleep(3)
        table_data = browser.find_element_by_id("queryLeftTable").find_elements_by_tag_name("tr")
        arr_dt=[]
        for p in table_data:
        ...
        for msg in arr_dt2:
            if len(msg)>1:
                ...
                shu=shu+1
        if pao!=10:
            browser.find_elements_by_link_text("预订")[pao].click()

            time.sleep(3)
            browser.find_element_by_link_text("账号登录").click()
            time.sleep(1)
            browser.find_element_by_id("J-userName").send_keys("")  #账号
            time.sleep(3)
            browser.find_element_by_id("J-password").send_keys("")  # 密码
            time.sleep(5)
            browser.find_element_by_link_text("立即登录").click()
            time.sleep(2)
            browser.find_element_by_link_text("短信验证").click()
            time.sleep(2)
            browser.find_element_by_id("id_card").send_keys("6016")
            time.sleep(2)
            browser.find_element_by_link_text("获取验证码").click()
            time.sleep(60)
            # 打开apk应用
            # 等待验证码
            dd = json.loads(requests.get("http://port.wapjin.com/cat_msg?strAddress=12306").text)["data"]
            code = dd[len(dd) - 1:][0][1][dd[len(dd) - 1:][0][1].find("验证码：") + 4:dd[len(dd) - 1:][0][1].find("验证码：") + 10]
            time.sleep(2)
            browser.find_element_by_id("code").send_keys(code)
            time.sleep(2)
            browser.find_element_by_link_text("确定").click()
            time.sleep(3)
            browser.find_element_by_id("djPassenger_0").click()
            time.sleep(3)
